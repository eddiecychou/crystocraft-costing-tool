...
import { loadComponents, saveComponents } from '../criticalComponents'
...
function ComponentsCard() {
  const [rows, setRows] = useState([])   // [{code,name,stock_qty,lead_time_weeks}]
  const [saved, setSaved] = useState([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [msg, setMsg] = useState(null)
  ...
  useEffect(() => {
    loadComponents().then(c => {
      const r = c.map(x => ({ ...x, stock_qty: x.stock_qty ?? '', lead_time_weeks: x.lead_time_weeks ?? '' }))
      setRows(r); setSaved(r); setLoading(false)
    })
  }, [])

  const update = (i, key, val) => setRows(rs => rs.map((r, j) => (j === i ? { ...r, [key]: val } : r)))
  const addRow = () => setRows(rs => [...rs, { code: '', name: '', stock_qty: '', lead_time_weeks: '' }])
  const removeRow = i => setRows(rs => rs.filter((_, j) => j !== i))
  const move = (i, dir) => setRows(rs => {
    const j = i + dir
    if (j < 0 || j >= rs.length) return rs
  ...
  async function handleSave() {
    setSaving(true); setMsg(null)
    try {
      const clean = await saveComponents(rows)
      const r = clean.map(x => ({ ...x, stock_qty: x.stock_qty ?? '', lead_time_weeks: x.lead_time_weeks ?? '' }))
      setRows(r); setSaved(r)
      setMsg(`Saved ${r.length} component${r.length === 1 ? '' : 's'}.`)
      setTimeout(() => setMsg(null), 3000)
    } catch (e) {
      setMsg('Error saving: ' + e.message)
    } finally { setSaving(false) }
  ...
  return (
    <div className="card p-5 mb-4">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-sm font-semibold text-gray-700">Critical Components Library</h2>
        <button onClick={addRow} className="btn-secondary text-xs py-1.5 px-3">+ Add component</button>
      </div>
      <p className="text-xs text-gray-400 mb-4">
        Only <strong>business-critical</strong> parts that drive the production promise — long lead,
        tooling, MOQ, or supply risk (figurine bodies, metal parts, music-box movements). Stock is
        shared: one body pool feeds every product that uses it. Don't list commodity parts (plating,
        crystal, gift boxes) — those are fast replenishment and don't change delivery.
      </p>

      {loading ? (
        <p className="text-xs text-gray-400">Loading…</p>
      ) : rows.length === 0 ? (
        <p className="text-xs text-gray-400">No components yet — add the long-lead / tooling parts your figurines share.</p>
      ) : (
        <div className="space-y-2">
          <div className="hidden sm:flex items-center gap-2 text-[10px] uppercase tracking-wide text-gray-400 px-1">
            ...
            <span className="w-20 shrink-0 text-right">Lead wk</span>
            <span className="w-16 shrink-0" />
          </div>
          {rows.map((r, i) => (
            <div key={i} className="flex items-center gap-2">
              <input className="input text-xs font-mono uppercase w-24 shrink-0" value={r.code}
                     placeholder="U0002-BODY" maxLength={16}
                     onChange={e => update(i, 'code', e.target.value.replace(/[^A-Za-z0-9-]/g, '').toUpperCase())} />
              <input className="input text-xs flex-1 min-w-0" value={r.name}
                     placeholder="Owl body / 18-note music box" onChange={e => update(i, 'name', e.target.value)} />
              <input className="input text-xs w-20 shrink-0 text-right tabular-nums" value={r.stock_qty}
                     inputMode="numeric" placeholder="0"
                     onChange={e => update(i, 'stock_qty', e.target.value.replace(/[^\d]/g, ''))} />
              <input className="input text-xs w-20 shrink-0 text-right tabular-nums" value={r.lead_time_weeks}
                     inputMode="numeric" placeholder="wk"
                     onChange={e => update(i, 'lead_time_weeks', e.target.value.replace(/[^\d.]/g, ''))} />
              <div className="flex items-center gap-0.5 w-16 shrink-0 justify-end">
                <button type="button" onClick={() => move(i, -1)} disabled={i === 0}
                        className="text-gray-400 hover:text-gray-700 disabled:opacity-30 px-1" title="Move up">↑</button>
                <button type="button" onClick={() => move(i, 1)} disabled={i === rows.length - 1}
