...
import { Mail, Clock, CheckCircle2, XCircle, Ban, RefreshCw, Loader2 } from 'lucide-react'
...
export default function PortalInvitations({ embedded = false }) {
  ...
  useEffect(() => {
    return onSnapshot(collection(db, 'customers'), snap => {
      setCustomers(Object.fromEntries(snap.docs.map(d => [d.id, d.data()])))
    })
  }, [])
  ...
  const content = (
    <div className={embedded ? '' : 'p-4 md:p-6'}>
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-400">No invitations yet. Invite a customer contact from their record in Customers.</p>
      ) : (
        <div className="card divide-y divide-gray-100">
          {invitations.map(inv => {
            const st = STATUS_STYLE[inv.status] || STATUS_STYLE.pending
            const company = customers[inv.customer_id]?.company_name || inv.customer_id
            const busy = busyId === inv.id
            return (
              <div key={inv.id} className="px-4 py-3 flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-gray-800">{inv.contact_name || inv.contact_email}</p>
                  <p className="text-xs text-gray-500">{inv.contact_email} · {company}</p>
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    Invited {fmtDate(inv.created_at)} by {inv.created_by || '—'}
                    {inv.claimed_at && ` · Claimed ${fmtDate(inv.claimed_at)}`}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {busy ? (
                    ...
                    <>
                      {inv.status === 'claimed' && (
                        <>
                          <button onClick={() => run(approveInvitation, inv.id)} className="btn-primary text-xs py-1 px-2">Approve</button>
                          <button onClick={() => run(rejectInvitation, inv.id)} className="btn-secondary text-xs py-1 px-2">Reject</button>
                        </>
                      )}
                      {inv.status === 'pending' && (
                        <>
                          <button onClick={() => run(resendInvitation, inv.id)} className="btn-secondary text-xs py-1 px-2 inline-flex items-center gap-1">
                            <RefreshCw size={11} /> Resend
                          </button>
                          <button onClick={() => run(revokeInvitation, inv.id)} className="btn-secondary text-xs py-1 px-2 text-red-600">Revoke</button>
                        </>
                      )}
