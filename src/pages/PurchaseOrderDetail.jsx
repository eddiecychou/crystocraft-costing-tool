...
export default function PurchaseOrderDetail() {
  ...
  if (!po) return <div className="p-6 text-ink-60">Purchase order not found.</div>
    ...
    <div className="p-4 md:p-6 max-w-3xl">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mt-2 mb-6">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-2xl font-bold text-ink font-mono">{po.pu_number || '(no PU no.)'}</h1>
      ...
      <div className="card overflow-hidden mb-4">
        ...
        <div className="flex justify-end px-4 py-3 border-t border-warm-grey bg-ivory/50">
          <div className="w-64 space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-ink-60">Subtotal</span><span className="tabular-nums">{fmtMoney(totals.subtotal, cur)}</span></div>
            ...
            {totals.adjustmentsTotal !== 0 && (
              <div className="flex justify-between font-medium pt-1 border-t border-warm-grey"><span className="text-ink-70">Order Total</span><span className="tabular-nums">{fmtMoney(totals.grandTotal, cur)}</span></div>
            )}
            {totals.deposit > 0 && (
              <div className="flex justify-between text-ink-60"><span>Deposit ({po.deposit_pct}%)</span><span className="tabular-nums">− {fmtMoney(totals.deposit, cur)}</span></div>
            )}
      ...
      {po.remarks && (
        <div className="card p-5">
          <p className="text-xs text-ink-60 mb-1">Remarks</p>
