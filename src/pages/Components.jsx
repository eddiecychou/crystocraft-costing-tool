...
function StockEditor({ component: c, collectionPath = 'range_components' }) {
...
// ── Crystal Stock tab ────────────────────────────────────────────────────────
// Crystal SKUs (per colour, each an ERP code) held as inventory with the shared
// stock ledger. Batch-per-order consumption is issued from the order page; here
// we hold the master + on-hand + per-crystal ledger. Deliberately separate from
// Critical Components so crystals never enter the figurine MRP/BOM.

function CrystalStock() {
  const { crystals, loading } = useCrystals()
  const [search, setSearch] = useState('')
  ...
  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase()
    if (!q) return crystals
    return crystals.filter(c => [c.code, c.name, c.colour, c.size].some(v => (v || '').toLowerCase().includes(q)))
  }, [crystals, search])

  const totalOnHand = useMemo(() => crystals.reduce((s, c) => s + (Number.isFinite(c.stock_qty) ? c.stock_qty : 0), 0), [crystals])

  return (
    <div>
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <input className="input text-sm flex-1 min-w-[180px]" placeholder="Search code, name, colour…"
               value={search} onChange={e => setSearch(e.target.value)} />
      ...
      {adding && <CrystalAddRow onDone={() => setAdding(false)} />}

      <p className="text-xs text-ink-50 mb-2">
        {loading ? 'Loading…' : `${filtered.length} of ${crystals.length} crystal${crystals.length === 1 ? '' : 's'} · ${totalOnHand.toLocaleString()} pcs on hand`}
      </p>

      {!loading && crystals.length === 0 ? (
        <div className="card p-6 text-center text-sm text-ink-60">
          No crystals yet. <button onClick={() => setImporting(true)} className="text-brand-600 hover:underline">Import your crystal stock list</button> to seed the master, or add one.
        </div>
      ) : (
        <div className="card divide-y divide-ivory-dark overflow-hidden">
          {filtered.map(c => (
            <div key={c.id}>
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 px-3 py-2.5 hover:bg-ivory/50 transition-colors">
                <button onClick={() => setExpanded(e => e === c.id ? null : c.id)} className="flex items-center gap-3 min-w-0 flex-1 text-left">
                  ...
                  <Gem size={16} className="text-brand-400 shrink-0" />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-sm text-ink-90 truncate">{c.code}</span>
                      {c.colour && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-brand-50 text-brand-700 shrink-0">{c.colour}</span>}
                ...
                <div className="flex justify-end shrink-0 pl-14 sm:pl-0">
                  <StockEditor component={c} collectionPath="crystals" />
                </div>
              </div>
              {expanded === c.id && (
                <div className="px-3 pb-3 bg-ivory/30">
                  <StockLedger componentId={c.id} currentStock={c.stock_qty || 0} collectionPath="crystals" />
                  <button onClick={() => { if (window.confirm(`Delete crystal ${c.code}? Its ledger history stays but the SKU is removed.`)) deleteCrystal(c.id) }}
                          className="mt-2 inline-flex items-center gap-1 text-xs text-red-500 hover:text-red-700">
                    <Trash2 size={12} /> Delete crystal
                  </button>
                </div>
              )}
            </div>
      ...
      {importing && <CrystalImportModal onClose={() => setImporting(false)} />}
    </div>
  )
}

// Inline add form for a single crystal SKU. Stock is set afterwards via the ledger.
function CrystalAddRow({ onDone }) {
  const [f, setF] = useState({ code: '', name: '', colour: '', size: '' })
  const [saving, setSaving] = useState(false)
  const set = k => e => setF(x => ({ ...x, [k]: e.target.value }))
  async function save() {
    if (!f.code.trim()) return
    setSaving(true)
    try { await saveCrystal(null, f); onDone() } finally { setSaving(false) }
  }
  return (
    <div className="card p-3 mb-3 grid grid-cols-2 sm:grid-cols-4 gap-2 items-end">
      <div><label className="label text-xs">Code *</label><input className="input text-sm font-mono uppercase" value={f.code} onChange={e => setF(x => ({ ...x, code: e.target.value.toUpperCase() }))} placeholder="BDC-8232-0014-005" /></div>
      <div><label className="label text-xs">Name</label><input className="input text-sm" value={f.name} onChange={set('name')} placeholder="Rosaline" /></div>
      <div><label className="label text-xs">Colour</label><input className="input text-sm" value={f.colour} onChange={set('colour')} placeholder="PI" /></div>
...
// Parse a pasted crystal stock list. Each line: code, [name…], qty (tab or comma
// separated). code = first cell; qty = last purely-numeric cell; name = the rest.
function parseCrystalStock(text) {
  const out = []
...
function CrystalImportModal({ onClose }) {
  const [text, setText] = useState('')
  const [busy, setBusy] = useState(false)
  const [result, setResult] = useState(null)
  const rows = useMemo(() => parseCrystalStock(text), [text])

  async function run() {
    setBusy(true)
    try { setResult(await importCrystalStock(rows)) } finally { setBusy(false) }
  }

  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-lg w-full p-5 max-h-[85vh] overflow-y-auto">
        <h3 className="text-base font-semibold mb-1">Import crystal stock</h3>
        <p className="text-xs text-ink-60 mb-3">
          Paste rows as <span className="font-mono">code · name · qty</span> (tab or comma separated) — e.g. from the ERP stock report.
