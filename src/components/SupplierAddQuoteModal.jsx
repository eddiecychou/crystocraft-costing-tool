...
export default function SupplierAddQuoteModal({ supplier, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!selectedComponent) { setError('Please select a component.'); return }
    if (!form.unit_cost) { setError('Unit cost is required.'); return }
    setSaving(true)
    setError('')
    try {
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="border-t border-warm-grey pt-3 space-y-4">
            ...
            <div>
              ...
              {volumeTiers.map((t, i) => (
                <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2 items-center mb-2">
                  <input className="input py-1.5 text-sm" type="number" min="1" placeholder="Min qty"
                    value={t.min_qty} onChange={e => setVolumeTiers(prev => prev.map((r, j) => j === i ? { ...r, min_qty: e.target.value } : r))} />
                  <input className="input py-1.5 text-sm" type="number" step="0.01" min="0" placeholder="Unit cost"
            ...
            <div>
              ...
              <textarea className="input" rows={2} value={form.notes} onChange={set('notes')} placeholder="Any conditions, remarks…" />
            </div>
          </div>
