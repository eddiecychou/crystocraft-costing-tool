...
import { issueCrystalsForOrder, reverseCrystalIssue } from '../orderStock'
import { Gem, Plus, Trash2, RotateCcw, CheckCircle2, ChevronDown, ChevronRight } from 'lucide-react'

// Order → crystal issue card (V7.13a crystals-2). Manual, batch-per-order:
// crystals have no per-product BOM, so the operator picks the colours + the
// actual quantities this order consumed and issues them to the ledger. Same
// order-tagged, reversible pattern as the metal issue card.

const fmt = n => (Number.isFinite(Number(n)) ? Number(n).toLocaleString() : '0')
const newLine = () => ({ _uid: 'cl_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6), crystal_id: '', qty: '' })

export default function OrderCrystalIssue({ orderId, orderLabel }) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, 'orders', orderId), snap => {
      const d = snap.data() || {}
      setState({ issued: !!d.crystals_issued, at: d.crystals_issued_at || null, lines: d.crystal_issued_lines || [] })
    })
  }, [orderId])

  const byId = Object.fromEntries(crystals.map(c => [c.id, c]))
  ...
  async function doIssue() {
    const lines = rows
      .filter(r => r.crystal_id && Number(r.qty) > 0)
      .map(r => ({ crystal_id: r.crystal_id, code: byId[r.crystal_id]?.code || '', qty: Number(r.qty) }))
    if (!lines.length) { setError('Add at least one crystal and quantity.'); return }
    if (!window.confirm(`Issue ${lines.length} crystal line(s) for order ${orderLabel}? This deducts from crystal stock.`)) return
    setBusy(true); setError('')
    try { await issueCrystalsForOrder(orderId, orderLabel, lines); setRows([newLine()]) }
    catch (e) { setError(e.message || 'Issue failed.') }
    finally { setBusy(false) }
  }
  async function doReverse() {
    if (!window.confirm(`Reverse the crystal issue for order ${orderLabel}? Crystals go back to stock.`)) return
    setBusy(true); setError('')
    try { await reverseCrystalIssue(orderId, orderLabel) }
    ...
    <div className="card p-4">
      <button type="button" onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between gap-2 text-left">
        <span className="flex items-center gap-2 text-sm font-semibold text-gray-700">
          ...
          <Gem size={14} className="text-brand-400" /> Crystal stock
        </span>
        {state.issued
      ...
      {open && (
        <div className="mt-3">
          {state.issued ? (
            <>
              <p className="text-xs text-ink-50 mb-2">{state.lines.length} crystal line(s) deducted for this order.</p>
              <table className="w-full text-sm">
                <tbody className="divide-y divide-gray-50">
                  {state.lines.map((l, i) => (
                    <tr key={l.crystal_id || i}>
                      <td className="py-1.5 pr-2"><span className="font-mono text-xs">{l.code || byId[l.crystal_id]?.code || l.crystal_id}</span>{byId[l.crystal_id]?.colour ? <span className="text-ink-50"> · {byId[l.crystal_id].colour}</span> : ''}</td>
            ...
            <p className="text-sm text-ink-50 py-2">No crystals in stock yet — add them in Components → Crystal Stock first.</p>
          ) : (
            <>
              <div className="space-y-2">
                {rows.map(r => {
                  const c = byId[r.crystal_id]
                  ...
                  return (
                    <div key={r._uid} className="flex flex-col sm:flex-row gap-2 sm:items-center">
                      <select className="input text-sm flex-1" value={r.crystal_id} onChange={e => setRow(r._uid, { crystal_id: e.target.value })}>
                        <option value="">— pick crystal —</option>
                        {crystals.map(x => <option key={x.id} value={x.id}>{x.code}{x.colour ? ` · ${x.colour}` : ''}{x.name ? ` · ${x.name}` : ''}</option>)}
                      </select>
