    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-md shadow-lg w-full max-w-lg flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex gap-1 mt-3 border-b border-gray-100">
            {[['corp', 'Corp Gift'], ['range', 'Figurine']].map(([k, label]) => (
              <button key={k} onClick={() => setTab(k)}
                className={`px-3 py-1.5 text-sm font-medium -mb-px border-b-2 ${tab === k ? 'border-brand-600 text-brand-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
        ...
        <div className="overflow-y-auto px-2 py-2 flex-1">
          {tab === 'corp' ? (
            corp == null ? <p className="text-sm text-gray-400 text-center py-8">Loading components…</p>
            : corpFiltered.length === 0 ? <p className="text-sm text-gray-400 text-center py-8">No matching components.</p>
            : corpFiltered.map(c => (
              <button key={`${c.product_id}:${c.component_id}`} onClick={() => onPick({ type: 'corp', product_id: c.product_id, component_id: c.component_id, label: c.component_name })}
                      className="w-full text-left flex items-start gap-2 px-3 py-2 rounded hover:bg-gray-50">
                <Package size={15} className="text-gray-300 shrink-0 mt-0.5" />
                <span className="min-w-0">
                  <span className="block text-sm text-gray-800 truncate">{c.component_name}</span>
                  <span className="block text-xs text-gray-400 truncate">{c.product_name}</span>
                </span>
              </button>
            ))
          ) : (
            rangeFiltered.length === 0 ? <p className="text-sm text-gray-400 text-center py-8">No matching components.</p>
            : rangeFiltered.slice(0, 300).map(c => (
              <button key={c.id} onClick={() => onPick({ type: 'range', component_id: c.id, label: c.code || c.name })}
                      className="w-full text-left flex items-start gap-2 px-3 py-2 rounded hover:bg-gray-50">
                <Puzzle size={15} className="text-gray-300 shrink-0 mt-0.5" />
                <span className="min-w-0">
                  <span className="block text-sm font-mono text-gray-800 truncate">{c.code}</span>
                  <span className="block text-xs text-gray-400 truncate">{c.name}{c.category ? ` · ${c.category}` : ''}</span>
                </span>
              </button>
            ))
          )}
