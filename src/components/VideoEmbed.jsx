...
// hqdefault.jpg (and every smaller size below it) is always generated by
...
// so no amount of object-fit/cropping removes them (confirmed live,
// 2026-08-21: fixing the container's aspect ratio alone did NOT fix the
// thumbnail). maxresdefault.jpg is generated per-video at its real
// dimensions and is unpadded for most Shorts — but it doesn't exist for
// every video (real 404), so it's tried first with a fallback chain down
// to hqdefault, which is the one size YouTube guarantees for any indexed
// video.
...
export default function VideoEmbed({ url, title = 'Product video', className = '' }) {
  ...
  useEffect(() => {
    if (!id) return
    setThumbLevel(0) // reset the fallback chain — this is a NEW video id
    let alive = true
