...
export default function FavouritesPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="eyebrow text-ink-60 mb-1.5">Nothing saved yet</p>
  ...
  return (
    <div>
      <h1 className="text-xl md:text-2xl mb-4">Favourites <span className="text-ink-60 text-base">({items.length})</span></h1>
          ...
          return (
            <div key={`${it.type}-${it.id}`} className="mosaic-tile group flex flex-col">
              ...
              <div className="p-3 flex flex-col gap-1 flex-1">
                ...
                {it.code && <p className="text-xs text-ink-60 font-mono">{it.code}</p>}
                <div className="mt-auto pt-2 flex items-center gap-2">
                  {inCart ? (
                    <span className="text-xs px-3 min-h-[40px] inline-flex items-center rounded-none border border-ivory-dark text-ink-60">In enquiry</span>
                  ) : it.type === 'figurine' ? (
                    <Link to={to}
                  ...
                  <button onClick={() => fav?.toggle(it)} className="text-xs text-ink-60 hover:text-red-500 ml-auto min-h-[40px] inline-flex items-center px-1">Remove</button>
                </div>
              </div>
            </div>
          )
