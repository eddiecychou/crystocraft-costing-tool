...
- `users/{uid}` — one Firestore mirror of a Firebase Auth account: `role` (`admin`|`staff`|`customer`, V8.14 flat), `modules[]` (staff capability keys), `status`, `ws_discount_pct`, `pricing_group`, `customer_id`, `ui_lang` (`'en'`|`'zh-Hans'`, V8.14 — drives the partial Simplified-Chinese supply UI), and last-seen fields `last_login_at` / `login_count` (stamped by `authActivity.js` `stampLogin()` from `useAuthState`; feed `PortalLogins.jsx`). `last_login_backfilled: true` marks the 26 rows set from Firebase Auth `lastSignInTime` on 2026-09-10 (L-18) — `login_count` there is a floor of 1. Auth: admin full; self can create pending + two self-update paths — a broad edit that freezes 9 admin-mirrored fields (`role`/`status`/`modules`/`ws_discount_pct`/…, **not** `ui_lang`), and a stamp-only clause `diff().affectedKeys().hasOnly(['last_login_at','login_count'])` (V8.15, L-18 — the fragile 9-field chain was denying customer login stamps). Owned by `src/domain/customer.js` + portal-invite functions. → `customers` via `customer_id`.
...
- `orders/{id}` reserve fields — `component_lines` / `crystal_lines` / `packaging_lines` entries are `{ <idField>, code, qty }`; V8.15 adds an optional `adj_seq` (int) when a reserved line's qty is hand-edited before production-in (`adjustReservedLine` in `src/orderStock.js`, bumped every edit, part of the movement idempotency key).
...
- `seo_batches/{autoId}` — SEO control plane Step 2. DSH-prepared batches of intended WordPress writes (`{status, note, items:[{payload, before, validation, decision, result}]}`). Created by the `seo-batch` Node function (Admin SDK, Bearer-secret auth); the human sets per-item `decision` + `status` from `/seo-review`. Rules: admin read+update, create/delete denied (client can't create). See `docs/skills/SEO-CONTROL-PLANE.md`.
