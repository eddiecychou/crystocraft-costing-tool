...
- `weekly-digest.js` (`/api/weekly-digest`) — Dashboard "This Week" section (V8.15): one batched DeepSeek call turning several customers' already-gathered CRM Interaction Log + email text (last 7 days) into a short per-customer digest each. Auth: module `customers`. Called from `src/domain/weeklySummary.js`.
