...
- `weekly-digest.js` (`/api/weekly-digest`) — Dashboard "This Month" section (V8.15, 30-day window since 2026-09-17): one batched DeepSeek call turning several customers' already-gathered CRM Interaction Log + email text (last 30 days) into a short per-customer digest each. Auth: module `customers`. Called from `src/domain/weeklySummary.js`.
...
- `erp.js` (`/api/erp`) — Proxies curated JES ERP views (Supabase, service-role key) for the read-only legacy archive. Entities: `customer`, `supplier`, `item`, `sales_invoice`, `sales_order`, `purchase`, `warehouse`, `item_type`, `stock`, `item_history` (V8.14 — every invoice line an item appeared on + the parent invoice's date/customer/currency + a computed net unit price, for price-history / quoting; view `erp_item_sales_history`), `item_purchase_history` (V8.15 — the buy-side mirror: every purchase line an item appeared on + the parent PU's date/supplier/currency + computed net unit price; view `erp_item_purchase_history`), plus the `lines`/`bom`/`alternatives`/`codes`/`sync_status`/`item_images` sub-actions. The whitelisted-entity-search branch also accepts an optional `offset` (paging cursor alongside `limit`, added for `crystocraft-expenses`'s one-time legacy history import of `purchase`/`sales_invoice` — the only way to get past the first `limit`-sized page since there's no other cursor). Auth: module `erp` — **all-or-nothing** since V8.14: admin, or a `staff` account holding `erp`, gets the entire ERP surface. The V8.12 `production`/`sales` per-entity tiers (`PRODUCTION_ENTITIES`/`SALES_ENTITIES`) were removed with the role shim; `erp` is marked `sensitive` in `MODULE_GROUPS` (it exposes the full JES archive incl. costs/margins). **V8.15 exception:** the `item_purchase_history` entity alone is also reachable by a `staff` account holding `pricing` **or** `supply` (it exposes only what we paid for one item code — used by the Crystal costs list's inline PU-price check + its ERP Lookup tab — not the customer/margin surface). Called from `src/erpApi.js`, `src/customerOrderHistoryApi.js`, `src/pages/CustomerDetail.jsx`, and (entities `purchase`/`sales_invoice` only, one-time) `crystocraft-expenses`'s `sync-operation-center.js`.
- `uc.js` (`/api/uc`) — Read/write + atomic allocation for the UC# invoice registry (Supabase `uc_registry`). Auth: module `uc` (still an inline `isFrontOffice` copy — see `TECH-DEBT.md`). Called from `src/ucRegistry.js`, `src/pages/ShipmentForm.jsx`, `src/shipping.js`. `list_invoices` also accepts an optional `since` (ISO datetime) to filter `app_sales_invoice` by `updated_at` — added for the separate Finance app's sync (below), existing callers unaffected.
- `finance-po-sync.js` (`/api/finance-po-sync`) — Read-only summary of `purchase_orders` (Firestore), for the separate Finance / Bookkeeping app (`crystocraft-expenses`) to pull instead of a manual CSV export/import. Auth: module `supply`, same Firebase-ID-token verification as `uc.js`/`erp.js` — the caller is a dedicated service account Finance's own server signs in as. Optional `since` (ISO datetime) filters by each doc's Firestore `updateTime`. No writes. Called from `crystocraft-expenses`'s `netlify/edge-functions/sync-operation-center.js` (different repo).
...
- `download-image.js` (`/api/download-image`) — Forces a "Save As" download of a file in this app's own Storage bucket, or on crystocraft.com (WordPress-hosted figurine gallery photos — same carve-out as `image-proxy.js`, added 2026-09-21 per **L-28**) (works around cross-origin `<a download>` limits). Auth: public by necessity (plain anchor click, no Authorization header) — validates the URL against that allowlist instead. Called from 10+ export/gallery components.
...
- **Node function** `netlify/functions/seo-batch.js` (`/api/seo-batch`) — SEO control plane Step 2. DSH-facing: `create` / `poll` / `get` / `result` for change batches in Firestore `seo_batches`. Node (Admin SDK) like `portal-invite.js`. Auth: `Authorization: Bearer <SEO_BATCH_SECRET>` (machine, not a Firebase session). The human reviews via `src/pages/SeoReview.jsx` (`/seo-review`, admin, client SDK). Not one of the 37 edge functions. **V8.15:** `create` re-runs `seo-control-plane/validate-payload.mjs` server-side — the stored `validation` is the OC's authoritative result, DSH's self-report is kept as `dsh_validation` with a `validation_mismatch` flag, `create` returns `failed_validation` + `mismatches[]`, and `poll` downgrades any approved-but-OC-failed item to `decision:'blocked'` so it can't reach a write.
