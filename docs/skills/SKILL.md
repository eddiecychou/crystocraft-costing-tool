...
- **Item purchase history** tab (V8.15) — buy-side mirror: `erp` entity `item_purchase_history` / view `erp_item_purchase_history` (join `purchasedetail`→`purchase`): every PU line an item was bought on, with date/supplier/currency + `net_price`. Same `PriceSummary` (parametrised `groupField`), grouped by item+supplier+currency. Also reachable with the `pricing`/`supply` module (narrow exception in `erp.js`). Feeds the **Crystal costs** tab's inline "PU paid" readout: each `settings/crystal_unit_costs` row carries an optional one-time `erp_codes[]` link; `CrystalErpPrice` in `Components.jsx` shows the latest paid price + a "below last paid" flag + a "Use PU price" button. Reference only — never writes the costing engine.
