...
| `api_views.sql` | curated read-layer for the app: `erp_customer`, `erp_supplier` (views) + `erp_item` (materialized view, latest revision per code), the sales-invoice / sales-order / purchase header + line + surcharge views, `erp_stock`, `erp_item_sales_history` (V8.14 — invoice lines × their header, with a computed `net_price` = line total ÷ qty, for the ERP Lookup "Item price history" tab), and `erp_item_purchase_history` (V8.15 — the buy-side mirror: purchase lines × their PU header, same computed `net_price`, for the "Item purchase history" tab + the Crystal costs PU-price check). Every view must cast (raw is all `text`). Re-runnable. |
