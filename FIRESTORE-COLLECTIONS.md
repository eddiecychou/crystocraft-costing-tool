...
`firestore.rules` (lines 37–446) — that file is the authority on auth
posture; this doc is the map, not a substitute for reading a rule when it
actually matters (e.g. before changing one).

**Auth key:** *admin* = admin-only. *portal* = an approved, signed-in
customer can read their own data (never write, except the few marked). *self*
= the signed-in user's own doc/uid. *public* = written by an unauthenticated
caller, always through a specific edge function's service-account write, never
...
- `products/{id}` — storefront catalogue item. Auth: portal read, admin write. Owned by `src/domain/customer.js` + `Products.jsx`/`ProductForm.jsx`. → `range_products`, `orders`.
  - `products/{id}/images/{imageId}` — photos, screened per-customer via `branded_for_customer_id` vs the viewer's `sensitive` flag. → `customers`.
  - `products/{id}/pricing_tiers/{tierId}` — tier pricing. Auth: portal read, admin write.
  - `products/{id}/customer_prices/{priceUid}` — per-customer price overrides, keyed by uid.
  - `products/{id}/components/{componentId}` — BOM components. Auth: admin only.
    - `.../images/{imageId}` — component photos.
    - `.../supplier_quotes/{quoteId}` — quotes for that component (see collection-group rule below).
      - `.../attachments/{attachmentId}`.
- `range_products/{id}` (+ all subpaths above, mirrored) — catalogue ranges/families. Auth: portal read, admin write. Owned by `Range.jsx`, `RangeForm.jsx`.
...
- `settings/{docId}` — mixed config; an allowlist of docs (`catalogue_band`, `exchange_rates`, `format_moq`, `front_page`, `productDefaults`, `quote_branding`, `crystal_colors`) is portal-readable, everything else (`pricing_groups`, `crystal_unit_costs`, `components`) admin-only. Owned by `Settings.jsx`, `FrontPageConfig.jsx`, `PricingTiers.jsx`.
...
## Suppliers & procurement (admin only throughout)

- `suppliers/{id}` (+ `catalogs/{catalogId}`) — owned by `Suppliers.jsx`, `SupplierDetail.jsx`.
- `client_quotes/{id}` (+ `items/{itemId}`) — owned by `src/domain/customer.js` + `QuoteDetail.jsx`, `RangeQuoteForm.jsx`.
- `purchase_orders/{id}` — supplier POs, owned by `PurchaseOrderForm.jsx`, `PurchaseOrders.jsx`.
- `credit_notes/{id}` — combined sales-return + credit-note working doc — the *posted* financial fact lives in Supabase (`credit-note.js`/`app_credit_note`, see API-REFERENCE.md), not here. Owned by `CreditNoteForm.jsx`, `CreditNotes.jsx`.
...
- `{path=**}/supplier_quotes` — same pattern, for querying every `supplier_quotes` subcollection across products/components and range_components at once.

## Costing & catalogue documents (admin only)

- `catalogues/{id}` (+ `items/{itemId}`) — owned by `Catalogues.jsx`, `CatalogueForm.jsx`, `CataloguePreview.jsx`.
- `range_components/{id}` — component costing for ranges.
  - `.../supplier_quotes/{quoteId}` — covered by the collection-group rule above.
  - `.../movements/{movementId}` — append-only stock ledger (V7.13a) — see the inventory pattern below.

## Inventory (admin only, same doc + append-only `movements/{movementId}` ledger shape throughout)

- `crystals/{id}` — per-colour crystal SKUs.
- `crystal_swatch_notes/{colorId}` — curated legacy-Swarovski-reference metadata; **uniquely also portal-readable** (approved customers, not just admin) even though write stays admin-only.
- `packaging/{id}` — gift boxes/cartons/inserts.
- `b2c_stock/{id}` — retail finished-goods stock; per project memory, ChunCi's 卡斯库存 XLS is the real source, this collection doesn't connect to production.
...
- `uc_invoices/{id}` — UC# invoice registry mirroring app-generated invoices — the team's "PI"/"invoice"/JES-SO/SI distinctions from CLAUDE.md apply here.
- `counters/{name}` — atomic counters (e.g. `counters/uc` backs UC# allocation in `uc.js`).
