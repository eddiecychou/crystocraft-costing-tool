...
| `docs/skills/ARCHITECTURE-RULES.md` | Hard boundaries: data isolation, RBAC (V8.14 flat `admin`/`staff`/`customer` + per-user `users/{uid}.modules[]`; the fixed `production`/`sales` roles retired), data lifecycles, snapshots, verify/deploy playbook |
